...
def apply_watsonx_secrets() -> None:
    """Map Streamlit Cloud Secrets → environment for watsonx_client."""
    try:
        wx = st.secrets.get("watsonx", {})
        if wx:
            if wx.get("api_key"):
                os.environ["WATSONX_API_KEY"] = str(wx["api_key"])
            if wx.get("project_id"):
                os.environ["WATSONX_PROJECT_ID"] = str(wx["project_id"])
            if wx.get("url"):
                os.environ["WATSONX_URL"] = str(wx["url"]).rstrip("/")
...
def call_vision_api(image_base64):
    """Vision: FastAPI backend, or watsonx inline on Streamlit Cloud."""
    if test_backend():
        try:
            ...
            st.warning(f"Backend error, using watsonx directly: {e}")

    try:
        from watsonx_client import analyze_screenshot, credentials_configured

        if not credentials_configured():
            st.error("Configure [watsonx] in Streamlit Secrets or start the FastAPI backend.")
            return None

        tokens, usage = analyze_screenshot(image_base64)
        style_lock = {
            "locked": True,
            "tokens": tokens,
            "constraints": [
        ...
        return {"tokens": tokens, "styleLock": style_lock, "usage": usage}
    except Exception as e:
        st.error(f"watsonx Error: {str(e)}")
        return None


def call_generate_api(tokens, component_names, language="en"):
    """Generate: FastAPI backend, or watsonx inline on Streamlit Cloud."""
    if test_backend():
        try:
            response = get_session().post(
    ...
    try:
        from watsonx_client import credentials_configured, generate_text
...
5. Export a 'locked' property per component"""
        ...
        user_prompt = f"""Generate TypeScript React components for: {', '.join(component_names)}
...
Generate production-ready components with proper TypeScript interfaces."""
        ...
        return {"code": code, "language": language, "tokens": tokens}
    except Exception as e:
        st.error(f"watsonx Error: {str(e)}")
        return None
...
Powered by **IBM watsonx.ai (Granite)** • **IBM Bob** • **Vultr** • **Speechmatic**
""")

# Backend / watsonx status
backend_ok = test_backend()
try:
    from watsonx_client import credentials_configured

    watsonx_ok = credentials_configured()
except Exception:
    watsonx_ok = False
...
if backend_ok:
    ...
    st.success("✅ watsonx.ai connected (Granite via Streamlit Secrets)")
else:
    st.warning("⚠️ Add [watsonx] secrets or start FastAPI backend")
...
with col2:
    ...
    st.caption("IBM watsonx.ai (Granite) • IBM Bob • Vultr")
