...
# ============================================================================
# COMPLETE HEALTH APP HTML (Multi-Language) - FIXED F-STRING
# ============================================================================

def get_health_app_html(lang="en"):
    lang_data = LANGUAGES.get(lang, LANGUAGES["en"])
    
    # Use string formatting instead of f-string with JavaScript template literals
    html_template = '''
<!DOCTYPE html>
<html lang="{lang}">
<head>
    ...
    <title>{health_title} | IBM Bob</title>
    <style>
...
<div class="container">
    <div class="card" style="text-align:center; background: linear-gradient(135deg, #4f46e5, #7c3aed); color: white;">
        <h1 style="color:white;">🛡️ {health_title}</h1>
        <p>{welcome_text} - Track your health metrics and get AI-powered insights</p>
    </div>
    ...
    <div class="card">
        ...
        <button onclick="analyzeAndSave()" style="margin-top:1rem">{analyze_text} & Commit Data</button>
    </div>
...
<script>
    ...
    function displayHistory() {{
        ...
        for (let i = 0; i < Math.min(records.length, 10); i++) {{
            const r = records[i];
            html += "<tr><td><small>" + r.date + "</small></td><td>" + r.steps + "</td><td>" + r.water + "ml</td><td>" + r.sleep + "h</td><td><small>Head: " + r.headache + "<br>Body: " + r.bodyPain + "</small></td><td><span class=\"badge " + r.status + "\">" + r.status + "</span></td></tr>";
        }}
        html += "</tbody></table>";
...
'''
    ...
    return html_template.format(
        lang=lang,
        health_title=lang_data['health'],
        welcome_text=lang_data['welcome'],
        analyze_text=lang_data['analyze']
    )
                ...
                if (SpeechRecognition) {
                    startBtn.onclick = function() {
                        ...
                        recognition.onerror = function(event) {
                            statusText.innerHTML = '❌ Error: ' + event.error + '. Check microphone permissions.';
                        };
                        recognition.onend = function() {
                            statusText.innerHTML = '✅ Recording complete! Click Generate below.';
                        };
                        recognition.start();
                    };
