...
ibm_bob_client = anthropic.Anthropic(api_key=os.getenv("IBM_BOB_API_KEY"))
...
async def root():
    ...
    return {
        ...
        "version": "1.0.0",
        "description": "Backend API powered by IBM Bob",
        "docs": "/docs",
        "health": "/health",
        "endpoints": {
...
async def analyze_vision(request: VisionRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Call IBM Bob vision API
        response = ibm_bob_client.messages.create(
...
async def generate_components(request: GenerateRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Call IBM Bob with constraints
        response = ibm_bob_client.messages.create(
            model="claude-opus-4-20250514",
            max_tokens=4000,
            system=f"""You are a React component architect for App Architect Studio.
...
Breaking these constraints will cause rejection.""",
            messages=[
                {
                    "role": "user",
                    "content": f"""Generate TypeScript React components for: {', '.join(request.componentNames)}
...
async def fix_wordpress(request: dict, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        response = ibm_bob_client.messages.create(
            model="claude-opus-4-20250514",
            max_tokens=3000,
            system="""You are a WordPress theme specialist using IBM Bob.
...
6. Output fixed PHP code + CHANGELOG.md""",
            messages=[
                {
                    "role": "user",
                    "content": f"""Fix this WordPress issue: {problem}
...
async def startup_event():
    ...
    logger.info(f"IBM Bob API Key loaded: {bool(os.getenv('IBM_BOB_API_KEY'))}")
